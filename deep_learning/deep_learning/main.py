...
def train_model(model, train_loader, val_loader, criterion, optimizer, device , model_save_dir , num_epochs=100 , model_save_freq=10):
    ...
    for epoch in range(num_epochs):
        ...
        all_targets_flat = all_targets.view(num_sequences * num_joints , output_size).cpu().numpy()
        all_preds_flat = all_preds.view(num_sequences * num_joints , output_size).detach().cpu().numpy() 
        ...
        val_loss, val_mse, val_rmse, val_r2, val_pearson_corr = evaluate_model(model, val_loader, criterion, device, epoch=epoch, mode="Val", output_features = output_features)
...
I = 5
indicies = range(I , len(sequence_length_transformer) , 4)
...
if __name__ == "__main__":
    for i in range(len(sequence_length_transformer)):
        ...
        config["num_epochs"] = round(50 * config["secondary_sampling_rate_transformer"]/16)
        ...
        model_dir = f"{config['model_type']}_s{sequence_length}_sr{sampling_rate}_ssr{secondary_sampling_rate}_m{m}_h{num_heads}_d{embed_dim}_b{batch_size_training}_e{num_epochs}_v{version}"
        ...
        for chunk_index, robot_chunk in enumerate(get_robot_chunks(train_robots, chunk_size)):
            ...
            model.to(device)
            if config["continue"]:
                best_model_path = os.path.join(model_save_dir , "best", "model.pth")
                state_dict = torch.load(best_model_path, map_location=device, weights_only=True)
                model.load_state_dict(state_dict)

            criterion = nn.MSELoss()
            optimizer = optim.AdamW(model.parameters(), lr=lr, weight_decay=3e-6)
            ...
            train_model(model, train_loader, val_loader, criterion, optimizer, device, model_save_dir, num_epochs=num_epochs , model_save_freq=model_save_freq)
            ...
            test_loss, test_mse, test_rmse, test_r2 , test_pearson_corr = evaluate_model(model, test_loader, criterion, device, mode="Test" , output_features = output_features)
