...
def clean_and_format_urdf(folder_path , settings):
    ...
    # Gazebo plugin
    gazebo_plugin = ET.fromstring(f'''
    <gazebo>
        <plugin filename="libign_ros2_control-system.so" name="ign_ros2_control::IgnitionROS2ControlPlugin">
            <parameters>/root/ws/src/torque_controlled_arm/config/controllers.yaml</parameters>
            <controller_manager_node_name>/{folder_path.name}/controller_manager</controller_manager_node_name>
            <ros>
        ...
        if i != num_links - 1:
          ...
          if joint_tag is not None:
              ...
              dynamics_tag = ET.Element("dynamics", {
                  "damping": str(random.uniform(settings["viscous_friction_coeff_range"][0], settings["viscous_friction_coeff_range"][1])),
                  "friction": str(random.uniform(settings["coulomb_friction_coeff_range"][0], settings["coulomb_friction_coeff_range"][1]) * (settings["coulomb_friction_decay_factor"] ** i)) 
              })
...
def process_all_robot_folders(base_dir , settings):
    base_path = Path(base_dir)
    for folder in tqdm(base_path.iterdir(), desc="Processing robots", unit="robot"):
        if folder.is_dir():
            clean_and_format_urdf(folder, settings)
...
process_all_robot_folders(robots_dir , settings)
